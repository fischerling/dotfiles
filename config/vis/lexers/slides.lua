...
local latex_start_rule = lexer.starts_line(P(' ')^-3) * #P('\\') * latex:get_rule('environment') -- P(' ')^4 starts code_line
local latex_end_rule = #blank_line * ws
lex:embed(latex, latex_start_rule, latex_end_rule)

-- Embedded yaml.
local yaml = lexer.load('yaml')
local yaml_start_rule = #lexer.starts_line('---') * ws
local yaml_end_rule = #lexer.starts_line('---') * ws
